...
Required permission, capability, naming, recovery, or database failures block all file-changing execution. Read-only analysis may explain the issue and its remediation. An unavailable or unmounted local peer blocks before Source Inventory; the desktop Dry run must show those blockers immediately rather than an empty plan. SyncPlus never invokes `sudo`, changes ownership/permissions, or recommends broad permissions such as `chmod 777`.
