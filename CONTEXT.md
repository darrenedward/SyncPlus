...
**Sync workspace**:
The desktop surface for the active Sync Profile. A compact toolbar keeps the profile selector, Save, and Dry run visible. Folders, Options, and Plan are underline tabs on a baseline, not a row of ordinary selectable labels. Folders is one screen: a slim One-Way Sync / Mirror Sync strip, the task name, and source and destination folder pickers side by side with Browse. Options holds one Exclusion Rules list (file and folder patterns together, such as `*.tmp` and `node_modules/`) and, when Advanced Mode is on, Advanced safety options. Plan is the Dry run result: mapping, copy/overwrite/exclude counts, blockers, and Execution Confirmation. Live work uses a progress card with status, files, method, elapsed time, and the source-to-destination mapping. Browse opens the desktop folder picker as a child of the SyncPlus window so it stays in front.
_Avoid_: An Advance tab, two separate exclude-file and exclude-folder boxes, stacking source above destination so the setup requires scrolling, hiding one endpoint, moving confirmation into Options, or importing third-party mode names such as Basic Sync or Real-Time Sync
...
**Folder gate**:
Opening a saved Sync Profile runs a non-mutating availability check of local source and destination folders. For paths under `/mnt`, `/media`, or `/run/media`, SyncPlus reads the kernel mount table first; if the volume is not mounted it does not `stat` the path, because that wait can last a minute on an unplugged drive. Other paths use a short timed `stat`. The check does not walk the tree, measure transfer size, or show a running-job timer. If a folder is missing, unmounted, or unplugged, Dry run and Synchronise stay disabled. Retry on that folder section rechecks the same saved path. Dry run is for building a plan of folders that are present; it is not how a missing drive is recovered. SSH peers stay unproven until Dry run.
