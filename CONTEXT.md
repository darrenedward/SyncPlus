...
The shared core performs this comparison after each workflow run, persists the Source Inventory and typed findings in SQLite, and for Mirror persists one frozen inventory per peer. It exposes the affected paths and reasons through the Run Report. A missing current peer is recorded as Unavailable; a missing proof is Unverifiable; neither is converted into completion.
