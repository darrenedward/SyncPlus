...
**Sync workspace**:
The desktop surface for the active Sync Profile. A compact toolbar keeps the profile selector, Save, and Dry run visible. Folders, Options, and Plan are underline tabs on a baseline, not a row of ordinary selectable labels. Folders is one screen: a slim One-Way Sync / Mirror Sync strip, the task name, and source and destination folder pickers side by side with Browse. Options holds Exclusion Rules and, when Advanced Mode is on, Advanced safety options. Plan holds Fresh Analysis, Run Precheck, and Execution Confirmation. Live work uses a progress card with status, files, method, elapsed time, and the source-to-destination mapping.
_Avoid_: An Advance tab, stacking source above destination so the setup requires scrolling, hiding one endpoint, moving confirmation into Options, or importing third-party mode names such as Basic Sync or Real-Time Sync
