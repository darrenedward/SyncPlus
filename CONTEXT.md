...
**Folder gate**:
Opening a saved Sync Profile runs a non-mutating availability check of local source and destination folders. That check only asks whether the selected folders are present; it does not walk the tree, measure transfer size, or show a running-job timer. If a folder is missing, unmounted, or unplugged, Dry run and Synchronise stay disabled. Retry on the reconnect prompt rechecks the same saved path. Dry run is for building a plan of folders that are present; it is not how a missing drive is recovered. SSH peers stay unproven until Dry run.
_Avoid_: Auto-starting a Sync Run on open, showing a Dry-run progress card for the open-profile folder check, enabling Synchronise on a missing folder, or treating an empty plan as success
