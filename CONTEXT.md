...
**Run Precheck**:
The non-mutating validation phase before Analyze and Execution Confirmation. It checks the selected paths, source readability, destination writability, required removal permissions, path overlap, available space, Trash capacity when relevant, and remote capability when using SSH. Hard blockers prevent the run; clearly labeled warnings may require acknowledgement without silently changing the requested safety policy. An unavailable, unmounted, or missing local peer is a hard blocker. Dry run and Fresh Analysis must show that result immediately on Overview and Sync workspace; they must not look like an empty successful plan, and they must not inventory a peer that is not present.
